...
def ast_to_string(ast):
    ...
    if ast["tag"] == "ternary":
        return f"({ast_to_string(ast['condition'])} ? {ast_to_string(ast['true_branch'])} : {ast_to_string(ast['false_branch'])})"
...
##############################################################################################################
    ...
    if ast["tag"] == "ternary":
        condition_val, cond_status = evaluate(ast["condition"], environment)
        if cond_status == "exit":
            return condition_val, "exit"
        
        if is_truthy(condition_val):
            true_val, true_status = evaluate(ast["true_branch"], environment)
            return true_val, true_status
        else:
            false_val, false_status = evaluate(ast["false_branch"], environment)
            return false_val, false_status
    
  ################################ADDED SWITCH EVALUATE####################################
    
    if ast["tag"] == "switch":
        switch_expr, switch_status = evaluate(ast["switch"], environment) #evaluating switch
        case_values = ast["case_values"]  #getiing case values using ast for convenience

        it = 0

        while it < len(case_values): #while we are iterating through case valyes 
            casevalue, cvalue_status = evaluate(case_values[it], environment) #evaluating case value
            if switch_expr == casevalue:
                case_stmt_value, cstsmt_status = evaluate(ast["case_stmts"][it], environment) #evaluating case value
                return None, cstsmt_status #returning status after eval
            it += 1
        defaultvalue, default_status = evaluate(ast["default_stmts"] , environment) #if no match, return default
...
def test_evaluate_switch_statement(): ################NEEED TO DOUBLE CHECK THIS
    print("testing evaluate_switch_statement")
    equals(" switch(1111){ case (1111):{ x=1;} case (2222): {x=2;} default: { x=3; } }", {"x":1}, None, {"x":1})
    equals(" switch(2222){ case (1111):{ x=1;} case (2222): {x=2;} default: { x=3; } }", {"x":2}, None, {"x":2})
    equals(" switch(1234){ case (234):{ x=4;} case (456): {x=5;} default: { x=6; } }", {"x":6}, None, {"x":6})
...
def test_ternary_basic():
    print("testing ternary operator basic...")
    equals("5 > 3 ? 10 : 20", {}, 10)
    equals("5 < 3 ? 10 : 20", {}, 20)

def test_ternary_with_variables():
    print("testing ternary with variables...")
    equals("x > 5 ? x * 2 : x + 1", {"x": 10}, 20)
    equals("x > 5 ? x * 2 : x + 1", {"x": 3}, 4)

def test_ternary_nested():
    print("testing nested ternary...")
    equals("x > 10 ? 1 : x > 5 ? 2 : 3", {"x": 15}, 1)
    equals("x > 10 ? 1 : x > 5 ? 2 : 3", {"x": 7}, 2)
    equals("x > 10 ? 1 : x > 5 ? 2 : 3", {"x": 3}, 3)

def test_ternary_with_assignment():
    print("testing ternary with assignment...")
    environment = {"x": 10}
    code = "result = x > 5 ? 100 : 200"
    equals(code, environment, 100, {"x": 10, "result": 100})

def test_ternary_in_array():
    print("testing ternary in array...")
    equals("[1, (2>1 ? 99 : 0), 3]", {}, [1, 99, 3])

def test_ternary_with_strings():
    print("testing ternary with strings...")
    equals("x > 0 ? \"positive\" : \"negative\"", {"x": 5}, "positive")
    equals("x > 0 ? \"positive\" : \"negative\"", {"x": -5}, "negative")

def test_ternary_short_circuit():
    print("testing ternary short circuit evaluation...")
    environment = {}
    code = """
        x = 0;
        result = true ? 5 : (x = 10);
    ...
    assert environment["y"] == 0, "x should not be assigned in false branch"

def test_ternary_in_boolean_expression():
    print("testing ternary in boolean expression...")
    equals("(true ? 1 : 0) > 0 ? \"yes\" : \"no\"", {}, "yes")
    equals("(false ? 1 : 0) > 0 ? \"yes\" : \"no\"", {}, "no")
...
if __name__ == "__main__":
    ...
    test_ternary_basic()
    test_ternary_with_variables()
    test_ternary_nested()
    test_ternary_with_assignment()
    test_ternary_in_array()
    test_ternary_with_strings()
    test_ternary_short_circuit()
    test_ternary_in_boolean_expression()
