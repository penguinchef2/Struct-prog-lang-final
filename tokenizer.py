...
patterns = [
    ...
    [r"\/", "/"],
    [r"\%", "%"],
    [r"\?", "?"],  # ternary operator question mark - ADDED
    [r"\(", "("],
    [r"\)", ")"],
    [r"\{", "{"],
    [r"\}", "}"],
...
def test_ternary_tokens():
    print("testing ternary operator tokens...")
    t = tokenize("x > 5 ? 10 : 20")
    tags = [tok["tag"] for tok in t[:-1]]
    assert "?" in tags, "Expected '?' token in ternary expression"
    assert ":" in tags, "Expected ':' token in ternary expression"
