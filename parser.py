...
def parse_ternary_expression(tokens):
    """
    ternary_expression = logical_expression [ "?" expression ":" ternary_expression ]
    """
    ...
    if tokens[0]["tag"] == "?":
        tokens = tokens[1:]
        true_branch, tokens = parse_expression(tokens)
        
        if tokens[0]["tag"] != ":":
            raise Exception(f"Expected ':' in ternary operator at position {tokens[0]['position']}")
        tokens = tokens[1:]
        
        false_branch, tokens = parse_ternary_expression(tokens)
...
def test_parse_ternary_expression():
    ...
    ternary_expression = logical_expression [ "?" expression ":" ternary_expression ]
    """
    print("testing parse_ternary_expression...")
    
    ast, tokens = parse_ternary_expression(tokenize("x > 5 ? 10 : 20"))
    assert ast["tag"] == "ternary"
    assert ast["condition"]["tag"] == ">"
    assert ast["true_branch"]["tag"] == "number"
    assert ast["true_branch"]["value"] == 10
    assert ast["false_branch"]["tag"] == "number"
    assert ast["false_branch"]["value"] == 20
    
    ast, tokens = parse_ternary_expression(tokenize("x > 10 ? 1 : x > 5 ? 2 : 3"))
    assert ast["tag"] == "ternary"
    assert ast["false_branch"]["tag"] == "ternary"
...
def parse_assignment_expression(tokens):
    ...
    left, tokens = parse_ternary_expression(tokens)
...
def parse_switch_statement(tokens):
    ...
    switch_statement = "switch" "(" expression ")" "{" { "case" "(" expression ")" ":" statement_list } [ "default" ":" statement_list ] "}"
    """
    assert tokens[0]["tag"] == "switch" #if token is switch
    tokens = tokens[1:]
    ...
    if tokens[0]["tag"] != "{": #check if next token is {
        raise Exception(f"Expected '{{': {tokens[0]}")
    tokens = tokens[1:]
    if tokens[0]["tag"] != "case": #check if next token is case
        raise Exception(f"Expected 'case': {tokens[0]}")
    case_value = [] #creating a list to contain case values and statements
    case_stmts = []
    while tokens[0]["tag"] == "case": #while the tag is case, check for more case 
        tokens = tokens[1:]
        if tokens[0]["tag"] != "(":
            raise Exception(f"Expected '(': {tokens[0]}")
        tokens = tokens[1:]
